...
AuraSkillBuffer *MakeAuraSkillBuffer(struct Unit *unit) {
  ...
  int distance = 0;
  u8 unitIndex =
      unit->index; // Loading as unsigned to prevent faulty comparisons
  ...
  for (int i = 0; i < 0x100; ++i) {
    ...
    for (int j = 0; buffer->skills[j] != 0; ++j) {
      if (AuraSkillTable[buffer->skills[j]] &&
          count < gSkillTestConfig.auraSkillBufferLimit) {
...
// Loops through premade aura skill buffer to find matching aura skills within
// range
bool NewAuraSkillCheck(struct Unit *unit, u8 skillID, int allyOption,
                       int maxRange) {
  s8 (*pAllegianceChecker)(int, int) =
      ((allyOption & 1) ? AreUnitsAllied : IsSameAllegiance);
  ...
  for (int i = 0; auraBuffer[i].skillID; ++i) {
    if (auraBuffer[i].distance <= maxRange &&
        auraBuffer[i].skillID == skillID) {
